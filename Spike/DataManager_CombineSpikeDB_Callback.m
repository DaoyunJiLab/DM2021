...
if (nfile <= 1)
    ...
    pinfo = S.pinfo; data = S.data; S = []; data = rmfield(data, 'grouplist'); %load the first structure to the final (pinfo) structure

    %now load other files and check one by one to add to the main structure
    for (i = 2:nfile)
        S = load(dbfile{i}, '-mat');
        nowpinfo = S.pinfo; nowdata = S.data; S= []; nowdata = rmfield(nowdata, 'grouplist');
...
for (i = 1:nspike)
    checkstr{i} = pinfo.general.parmfile{i};
end
for (i = 1:nspikenow)
    checkstrnow = nowpinfo.general.parmfile{i};
