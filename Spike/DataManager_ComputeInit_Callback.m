function [pinfo, data] = DataManager_ComputeInit_Callback(pinfo, data, cellind, vv)
%compute initial variables of a database
if (~isempty(cellind))
   disp('----->Compute intitial variables of the database');
   disp('-------> compute firing properties');
   [pinfo, data] = DataManager_FindSpikeFiring(pinfo, data, cellind, vv);
  
   %get waveform properties
   disp('-------> compute waveform properties');
