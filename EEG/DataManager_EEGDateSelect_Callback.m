function DataManager_EEGDateSelect_Callback
%%select eeg filess from eeg database according to input criteria
...
%%    general.recarea = CA1
%%
hf = gcbf; pinfo = getappdata(hf, 'eeg'); data = getappdata(hf, 'eegdata');
...
elseif (strcmp(tagmark, 'deletegroup')) %to delete a group
   ...
   if (~isempty(groupind))
       ...
       if (strcmp(dd, 'Yes'))
           ...
           setappdata(hf, 'hgroup', hgroup); setappdata(hf, 'eegdata', data);
...
elseif (strcmp(tagmark, 'combinegroup')) %to combine selected group into another group
   ...
   if (isempty(ind))
       disp('-------> no EEGs after the combination');
   else
...
elseif (strcmp(tagmark, 'subtractgroup')) %to subtract a small one from a large group
   ...
   if (numel(groupsel)~=2)
       ...
       if (isempty(ind))
           disp('-------> no EEGs after the subtraction');
       else
           input = inputdlg({'Group name'}, 'Enter group name', 1);
           if (~isempty(input{1}))
           ngroup = numel(data.grouplist.groupname);
...
elseif (strcmp(tagmark, 'intersectgroup')) %to choose the intersection of 2 or more groups
   ...
   if (numel(groupsel)<2)
       ...
       if (isempty(ind))
           disp('-------> no EEGs after the reverse intersection');
       else
           input = inputdlg({'Group name'}, 'Enter group name', 1);
           if (~isempty(input{1}))
           ngroup = numel(data.grouplist.groupname);
...
%  ncrt = 2;
       ...
       for (kkk = 1:ncrt)
           parmfilenow{kkk} = DataManager_SearchSpikeDatabase(pinfo, crt{kkk}, 'general', 'eegfile', cellind);
       end
       parmfile = findcommoncell(parmfilenow);
       disp(['-----> ', num2str(numel(parmfile)), ' items match the criteria']);
       spikesel = 0*spikeselection;
       if (~isempty(parmfile))
          %mark spikes in pinfo that match the spikelist: 
          for (i = 1:nspike)
              if (~isempty(find(strcmp(parmfile, pinfo.general.eegfile{i}))))
                 spikesel(i) = 1;
              end
          end
       end
...
setappdata(hspike, 'htext', htext);
setappdata(hspike, 'selection', spikeselection);
