function DataManager_BehavUpdateDisplay_Callback
...
for (i = 1:numel(groupsel))
    spikeselectindex = union(spikeselectindex, bhdata.grouplist.groupindex{groupsel(i)});
end
...
for (i = 1:nfield) subfield{i} = fieldnames(behav.(fieldtitle{i})); end
...
for (xx = 1:numel(groupsel)) %if any groups selected
    ...
    spikeselectindex = bhdata.grouplist.groupindex{groupsel(xx)};
    valuecol = []; valuetitle = []; ncol = 1;
    valuetitle{ncol} = bhdata.grouplist.groupname{groupsel(xx)};
