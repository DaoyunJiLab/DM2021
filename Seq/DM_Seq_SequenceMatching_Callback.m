function DM_Seq_SequenceMatching_Callback
...
if ~strcmp(ee, '.spikedb')
    ...
    %input = {'PairRank'; 'OrderRank'}; [sel, ok] = listdlg('ListString', input);
    %if ok 
    %    seqparm.rankmode = input{sel};
...
if ok
   seqparm.sessionoption = 'FreeSequences';
   evsss = {'FreeSequences'; 'SlidingWindows'};
   [sss, ok] = listdlg('ListString', evsss, 'PromptString', 'Session sequencing option');
   if ok seqparm.sessionoption = evsss{sss};  end
end
if ok
...
if ok
    ...
    if ok
          ...
          %if ~isempty(strfind(seqparm.evtimingoption, '_time')) %%%%if use peak times of temporal rate curves
              input = inputdlg({'Enter temporal resolution (s)'; 'Enter smoothing option'; 'Enter smoothing sigma (s)'; 'Enter peak threshld (std)'}, ...
          ...
          if ~isempty(strfind(seqparm.eventtimingoption, '_space')) %%%%if use peak locations of spatial rate curves
              ...
              input = inputdlg({'Enter smoothing option'; 'Enter smoothing sigma (pixels)'; 'Enter peak threshld (std)'}, ...
                  'Spatial timing parameter Input', 2, {'10'; '0.05'}); 
              if (~isempty(input))
                 seqparm.spatres = 10; seqparm.spatsmoothopt = input{1}; seqparm.spatsigma = str2num(input{2}); seqparm.spatPthre = str2num(input{3});
              else
                 ok = 0;
              end
          end
...
if ok
      if strcmp(seqparm.sessionoption, 'SlidingWindows')%%use sliding windows
          input = inputdlg({'Enter min/max session window sizes (s)'; 'Enter window size step (s)'; 'Enter window shifting time'},...
                  'Session sliding window parameters', 3, {'1 20'; '1'; '1'}); 
              if (~isempty(input))
                    aa = str2num(input{1}); bb = str2num(input{2}); windowsize = aa(1):bb:aa(numel(aa)); %various wind sizes
                    seqparm.sessionwindowsize = aa(1):bb:aa(numel(aa)); 
                    seqparm.sessionwindowsizestep = bb;
                    seqparm.sessionwindowshifttime = str2num(input{3}); %%%window shiftting time
              else
                    ok = 0;
              end
      end
...
if ok
      if strcmp(seqparm.eventoption, 'SlidingWindows')%%events: use sliding windows
          if (~strcmp(seqparm.evtimingoption, 'RatePeak_space'))&&(~strcmp(seqparm.evtimingoption, 'Rate1stPeak_space'))
              input = inputdlg({'Enter event min/max session window sizes (s)'; 'Enter window size step (s)'; 'Enter window shifting time'},...
                  'Event sliding window parameters', 3, {'1 20'; '1'; '1'}); 
              if (~isempty(input))
                    aa = str2num(input{1}); bb = str2num(input{2}); windowsize = aa(1):bb:aa(numel(aa)); %various wind sizes
              ...
              input = inputdlg({'Enter event min/max window sizes (pixels)'; 'Enter window size step (pixels)'; 'Enter window shifting distance'},...
                  'Sliding window parameters', 3, {'100 1000'; '50'; '50'}); 
              if (~isempty(input))
                   aa = str2num(input{1}); bb = str2num(input{2}); windowsize = aa(1):bb:aa(numel(aa)); %various wind sizes
                   seqparm.eventminwindowsize = aa(1); seqparm.eventmaxwindowsize = aa(numel(aa)); 
                   seqparm.eventwindowsizestep = bb;
...
if ok && strcmp(seqparm.eventoption, 'FreeSequences')
       input = inputdlg({'Enter max sequence length (x tmplate length)'; 'Enter max sequence duration (s)'},...
                  'Sequence leng options', 2, {'2'; '30'}); 
       if (~isempty(input))
...
if ok
    ...
    if (~isempty(seq))
       ...
       %pinfo = seq; data = seqdata; save(seqfile, 'pinfo', 'data', '-mat');
    end
end
disp('************************');
...
for (i = 1:numel(unifinaldir))
    ...
    elseif (numel(cellparmfile)<4)
        ...
        for (j = 1:numel(tmpnow)) %%%for each template
            ...
            [sessseq, evseq, tmpnow(j)] = sequencedata(pinfo, data, behav, bhdata, neuronid, tmpnow(j), seqparm, rrank, fdirnow);
...
function [sessseq, evseq, tmpnow] = sequencedata(pinfo, data, behav, bhdata, neuronid, tmpnow, seqparm, rrank, finaldirnow)
...
for (i = 1:nsess) %%%for session data the only available timingoption is 'RatePeak_time'
    ...
    if strcmp(seqparm.sessionoption, 'SlidingWindows') %%if sliding windows
        ep = []; mm = 1;  nn = numel(seqparm.sessionwindowsize);  ep = cell(mm,nn);
        for (ii = 1:mm)
             winshift = sessST(i):seqparm.sessionwindowshifttime:sessET(i); pp = numel(winshift);
             for (jj = 1:nn)
                   ep{ii,jj}.start = winshift - seqparm.sessionwindowsize(jj)/2;
                   ep{ii,jj}.ent = winshift + seqparm.sessionwindowsize(jj)/2;
                   for (k = 1:pp) ep{ii,jj}.marker{k} = sessName{i}; end
             end
        end
    elseif strcmp(seqparm.sessionoption, 'FreeSequences')
        ep = []; nep = 1;
        for (ti = 1:nep)
    ...
    [mm,nn] = size(ep);
    if strcmp(seqparm.sessionoption, 'FreeSequences')
...
%                breakallsequences(allsessletter{i}, allsesstime{i}, ep{ii,jj}, rrank, tmpseq);
    ...
    elseif strcmp(seqparm.sessionoption, 'SlidingWindows')
        for (ii = 1:mm) %%%original event index
        for (jj = 1:nn)
            [seq{ii,jj}, seqstart{ii,jj}, seqend{ii,jj}, seqmarker{ii,jj}, matchscore{ii,jj}, ...
                posmatchprob{ii,jj}, negmatchprob{ii,jj}] = ...
                findallsequence(ratedata{i}, cellletter, seqparm.sessiontimingoption, ...
                ep{ii,jj}, timePeakTh, timepoint{i}, rrank, tmpseq);
        end
        end
    end
...
%            [seq{ii,jj}, seqstart{ii,jj}, seqend{ii,jj}, seqmarker{ii,jj}, matchscore{ii,jj},...
%                posmatchprob{ii,jj}, negmatchprob{ii,jj}] = ...
%                breakallsequences(allevletter{i}{ii}, allevtime{i}{ii}, ep{ii,jj}, rrank, tmpseq);
        end
        end
...
%            [seq{ii,jj}, seqstart{ii,jj}, seqend{ii,jj}, seqmarker{ii,jj}, matchscore{ii,jj},...
%                posmatchprob{ii,jj}, negmatchprob{ii,jj}] = ...
%                breakallsequences(allevletter{i}{ii}, allevtime{i}{ii}, ep{ii,jj}, rrank, tmpseq); %%then break down the sequence to shoter ones with each event
        end
        end
...
posN = NaN*ones(1, nsess); posP = NaN*ones(1, nsess); posE = NaN*ones(1, nsess); posRate = NaN*ones(1, nsess); posSig = NaN*ones(1, nsess); posNSig = NaN*ones(1, nsess);
negN = NaN*ones(1, nsess); negP = NaN*ones(1, nsess); negE = NaN*ones(1, nsess); negRate = NaN*ones(1, nsess); negSig = NaN*ones(1, nsess); negNSig = NaN*ones(1, nsess);
for (i = 1:nsess)
    seqdata.data.sessseq{i} = sessseq{i};
    [posN(i), posP(i), posE(i), negN(i), negP(i), negE(i), posSig(i), negSig(i)] = findtheorysig(sessseq{i}, seqparm.significancelevel, rrank, seqparm.sessionoption);
...
seq.seq.sessPosMatchN = posN; seq.seq.sessPosMatchNtheosig = posNSig; seq.seq.sessPosMatchTheoP = posP; 
seq.seq.sessPosTheoExpN = posE; seq.seq.sessPosTheoExpSig = posSig; seq.seq.sessPosRate = posRate; 
seq.seq.sessNegMatchN = negN; eq.seq.sessNegMatchNtheosig = negNSig; seq.seq.sessNegMatchTheoP = negP; 
seq.seq.sessNegTheoExpN = negE; seq.seq.sessNegTheoExpSig = negSig; seq.seq.sessNegRate = negRate; 
...
% for (i = 1:nev)
%     seqdata.data.evseq{i} = evseq{i};
%     [posN(i), posP(i), posE(i), negN(i), negP(i), negE(i), posSig(i), negSig(i)] = findtheorysig(evseq{i}, seqparm.significancelevel, rrank, seqparm.eventoption);
%     totlap = numel(evTimes{i}.start); tottime = sum(evTimes{i}.ent-evTimes{i}.start);
%     posTimeRate(i) = posN(i)/tottime; negTimeRate(i) = negN(i)/tottime;
%     posLapRate(i) = posN(i)/totlap; negLapRate(i) = negN(i)/totlap;
%     posNSig(i) = (posN(i)-posE(i))/posSig(i); negNSig(i) = (negN(i)-negE(i))/negSig(i);
% end
% seq.seq.evPosMatchN = posN; seq.seq.evPosMatchNsig = posNSig; seq.seq.evPosMatchTheoP = posP; seq.seq.evPosTheoExpN = posE; seq.seq.evPosTheoExpSig = posSig;
% seq.seq.evPosTimeRate = posTimeRate; seq.seq.evPosLapRate = posLapRate; 
% seq.seq.evNegMatchN = negN; seq.seq.evNegMatchNsig = negNSig; seq.seq.evNegMatchTheoP = negP; seq.seq.evNegTheoExpN = negE; seq.seq.evNegTheoExpSig = negSig;
% seq.seq.evNegTimeRate = negTimeRate; seq.seq.evNegLapRate = negLapRate;

%%%%%shuffle significance
posP = NaN*ones(1, nsess); posE = NaN*ones(1, nsess); negP = NaN*ones(1, nsess); negE = NaN*ones(1, nsess); 
posRate = NaN*ones(1, nsess); negRate = NaN*ones(1, nsess); 
posSig = NaN*ones(1, nsess); negSig = NaN*ones(1, nsess);
for (i = 1:nsess) %for sessions
    alltimenow = sessseq{i}.alltime;
...
seq.seq.sessPosMatchNshufZ = (seq.seq.sessPosMatchN - posE)./posSig; seq.seq.sessPosMatchShufP = posP; seq.seq.sessPosShufExpN = posE; 
seq.seq.sessPosShufExpSig = posSig; seq.seq.sessShufposRate = posRate;
seq.seq.sessNegMatchNshufZ = (seq.seq.sessNegMatchN - negE)./negSig; seq.seq.sessNegMatchShufP = negP; seq.seq.sessNegShufExpN = negE; 
...
seq.seq.evPosMatchNshufZ = (seq.seq.evPosMatchN - posE)./posSig; seq.seq.evPosMatchShufP = posP; seq.seq.evPosShufExpN = posE; 
seq.seq.evPosShufExpSig = posSig; seq.seq.evShufposTimeRate = posTimeRate; seq.seq.evShufposLapRate = posLapRate;
seq.seq.evNegMatchNshufZ = (seq.seq.evNegMatchN - negE)./negSig; seq.seq.evNegMatchShufP = negP; seq.seq.evNegShufExpN = negE; 
...
maxTleng = seqparm.maxtmpleng; maxchecktime = seqparm.maxtime; %30 s
%%%%%break long
...
if ((max(Ctime)-min(Ctime))<=1.5*(max(tmprank)-min(tmprank))) && (max(abs(diff(Ctime)))<=1.5*max(abs(diff(tmprank)))) %%%%time span/gap restriction 
...
%           Cstrnow = Cstr(iii); shufscore(j) = findnewrankscore(Cstrranknow, Cstrnow, Ctime);
%       end 
...
% function Cstrrank = computeCrank(Cstr, Ctime)
% neq = numel(Cstr); Cstrrank = zeros(1, neq); 
% [~, iii] = sort(Ctime); Cstrrank(iii) = 1:neq; %%%%ordered rank
% Cutime = unique(Ctime); nuq = numel(Cutime); %%%resolve exactly peak times
% if (nuq < neq)
%     for (i = 1:nuq)
%         ii = find( Ctime == Cutime(i) );
%         if (numel(ii)>1)
%             Cstrrank(ii) = mean(Cstrrank(ii))*ones(1,numel(ii));
%         end
%     end
% end
% function score = findscore_multiple(Cstr, Cstrrank, tmpseq, tmprank)
% score = NaN;
% nq = numel(tmpseq); PP = 0; NN = 0; Cmeanrank = NaN*ones(1,nq);
% for (i = 1:nq) 
%     iii = find(Cstr == tmpseq(i)); Cmeanrank(i) = mean(Cstrrank(iii)); 
% end
% for (i = 1:nq-1)
%      for (j = i+1:nq)
%            tmpbias = tmprank(j)-tmprank(i);
%            Cstrbias = Cmeanrank(j) - Cmeanrank(i);
%            rawscore = tmpbias*Cstrbias;
%            if (rawscore>=0)
%                   PP = PP +1;
%            elseif (rawscore < 0)
%                   NN = NN + 1;
...
%      end
% end
% if (PP+NN>0) score = (PP-NN)/(nq*(nq-1)/2); end
% function score = findscore_multiple(Cstr, Cstrrank, tmpseq, tmprank)
% score = NaN;
% nq = numel(Cstr); PP = 0; NN = 0; tmpind = zeros(1,nq);
% for (i = 1:nq) tmpind(i) = find(tmpseq == Cstr(i)); end
% for (i = 1:nq-1)
%      for (j = i+1:nq)
%            if Cstr(i)~= Cstr(j)
%               tmpbias = tmprank(tmpind(j))-tmprank(tmpind(i));
%               Cstrbias = Cstrrank(j) - Cstrrank(i);
%               rawscore = tmpbias*Cstrbias;
%               if (rawscore>=0)
%                   PP = PP +1;
%               else
...
function [seq, seqstart, seqend, seqmarker, score, posprob, negprob] = ...
               breakallsequences(allletter, alltime, event, rrank, tmpseq)
%%%%%break long words into pairs, triplets, etc, breaks whenever see a repeating letter
iii = find( (alltime>=event.start) & (alltime<=event.ent) ); 
nletter = numel(iii); letternow = allletter(iii); timenow = alltime(iii); 
seq = []; seqstart = []; seqend = []; nseq = 0;
for (i = 1:nletter-1)
    sid = i; eid = i+1;
    while (eid <= nletter) && isempty(find(letternow(sid:eid-1) == letternow(eid)))
        nseq = nseq + 1; seq{nseq} = letternow(sid:eid); seqstart(nseq) = timenow(sid); seqend(nseq) = timenow(eid);
        eid = eid + 1;
    end
end
nseq = numel(seq); score = NaN*ones(nseq,1); posprob = NaN*ones(nseq,1); negprob = NaN*ones(nseq,1); seqmarker = cell(nseq, 1);
...
for (i = 1:nseq) seq{i} = char(seq{i}); seqmarker{i} = event.marker; end
seq = seq'; seqstart = seqstart'; seqend = seqend';
...
for (i = 1:nspike)
    ...
    iii = find( maxV > PeakTh(i) );  peakT{i} = timepoint(maxind(iii)); peakL{i} = matchletter(i)*ones(1,numel(iii));
    %peakT{i} = timepoint(maxind); peakL{i} = matchletter(i)*ones(1,numel(maxind)); %%%no peak filtering
end
allL = cell2mat(peakL); allT = cell2mat(peakT);
[alltime, iii] = sort(allT); allletter = allL(iii);
...
IXX = randperm(nspike); spikedata = spikedata(IXX); PeakTh = PeakTh(IXX); letter = letter(IXX);
...
for (i = 1:nev)
    %seqstart(i) = event.start(i); seqend(i) = event.ent(i); 
    seqmarker{i} = event.marker{i};
end
for (i = 1:nev)
...
if (numel(posid)~=1)||(numel(evid)~=1)
       ...
       for (jnow = 1:nlap)
            ...
            [spatrate, ~] = getlinearmap(spiketime, evok, lappostime, lapx, spatX, occutimenow, sigma, Nsigma);
       end 
end
function [D1rate, nact] = getlinearmap(spiketime, evTime, lappostime, lapx, xbin, occutime, smoothopt, sigma, Nsigma)
...
[mm,nn] = size(evseq.seq); seq = []; candN = 0;
for (i = 1:mm)
            ...
            for (j = 1:numel(sequence{ii,jj}))
                 ...
                 if isempty(find(ind == 0))
                    ...
                    if strcmp(eventoption, 'FreeSequences')
                       ...
                       %disp([seq{i}{ii,jj}{j}, '__', num2str(size(Ctime))]);
