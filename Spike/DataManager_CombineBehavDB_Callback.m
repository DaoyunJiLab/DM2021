function DataManager_CombineBehavDB_Callback
...
disp('Combine behavior database files');
startpath = cd;
[dbfile, filename, filepath, fileext, ok] = FileDlg(startpath, '.behavdb');  %get a group of database files

nfile = numel(dbfile);
if (nfile <= 1)
    disp('-----> less than 2 behavior database files selected; action aborted');
else
    ...
    pinfo = S.behav; data = S.bhdata; S = []; data = rmfield(data, 'grouplist'); %load the first structure to the final (pinfo) structure

    %now load other files and check one by one to add to the main structure
    for (i = 2:nfile)
        S = load(dbfile{i}, '-mat');
        nowpinfo = S.behav; nowdata = S.bhdata; S= []; nowdata = rmfield(nowdata, 'grouplist');
        [pinfo, data] = CheckAndAddDB(pinfo, nowpinfo, data, nowdata);
    end
    %re-create the default grouplist
    data.grouplist.groupname{1} = 'List0'; data.grouplist.groupindex{1} = 1:numel(pinfo.general.sessname);
    data.grouplist.grouptype{1} = 'Manual'; data.grouplist.groupcrit{1} = [];
    
    disp('-----> writing data back to a file');
    [fname, pname] = uiputfile(fullfile(cd, '*.behavdb'), 'Write combined spike database to:');
    writefilename = fullfile(pname, fname);
    behav = pinfo; bhdata = data; pinfo = []; data = [];
...
nspike = numel(pinfo.general.sessname); nspikenow = numel(nowpinfo.general.sessname);
for (i = 1:nspike)
    checkstr{i} = strcat(pinfo.general.finaldir{i}, '_', pinfo.general.sessname{i});
end
for (i = 1:nspikenow)
    checkstrnow = strcat(nowpinfo.general.finaldir{i}, '_', pinfo.general.sessname{i});
