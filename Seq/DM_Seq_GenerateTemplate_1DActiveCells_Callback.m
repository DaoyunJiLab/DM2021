function DM_Seq_GenerateTemplate_1DActiveCells_Callback
%------------This is for group-generation of linear track templates%%%%%%%%%%%%%%%%%%
%------------Generate templates on all active (removed if not within rate parameters) cells in selected events
%------------   redo 1D rate maps here to make sure that all cells are included
%------------   cells are ordered into sequences anyway even if no clear
%------------          peaks maybe produced (thus not accurate for sequence analysis, but still good for Bayesian decoding)
%%%Generate template based on (lap-averaged) firing curves (on linear track) in a spikedb 
%%%Template defined as binned spike count of multiple, current selective spike trains, regardless whether place fields can be defined.
%%%Spatial bin size is pre-defined in the figure-associated data sructure; temporal binsize is set here for producing the templates in the temporal
%%%     domain (not commonly used, but useful when compression factor needs to be computed)
...
if ok
    input = inputdlg({'Min rate (Hz)'; 'Max rate'; 'Time binsize(s)'}, 'Cell/time parameters', 3, {'0.5'; '10'; '0.1'}); 
    if (~isempty(input))
        tmpparm.minrate = str2num(input{1}); tmpparm.maxrate = str2num(input{2}); tmpparm.timebin = str2num(input{3});  
    else
        ok = 0;
    end
end
if ok
    input = inputdlg({'Smooth 1D rate maps?'; 'Sigma (bins)'; '# of sigmas'}, 'Smoothing parameters', 3, {'Yes'; '3'; '5'}); 
    if (~isempty(input))
...
tmp = []; ncell = 0; timebin = tmpparm.timebin; %%%default time binsize for temporal template generation
...
if (ncell<4)
    ...
    tmp.tmp.tmptype = 'pfratetmp'; tmp.tmp.tmplength = tmplength;
    ...
    %     %%%%parameters
    tmp.parm.epindex = NaN; tmp.parm.starttime = 0; tmp.parm.endtime = 0;  tmp.parm.minrate = tmpparm.minrate; tmp.parm.maxrate = tmpparm.maxrate;
...
otime = ones(size(occutime)); %disp([size(D1rate); size(occutime)]); disp('####');
