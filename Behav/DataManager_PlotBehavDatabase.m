function DataManager_PlotBehavDatabase(hmain, behav, bhdata)
%%plot a behav bhdatabase structure behav in figure hmain
...
hspike = TextDisplayer(hmain, spikelistpos, behav.general.sessID, 'Session', 'normalized');
hgroup = TextDisplayer(hmain, grouplistpos, bhdata.grouplist.groupname, 'Group', 'normalized');
...
spikeselectindex = bhdata.grouplist.groupindex{1};
...
for (xx = 1:numgroup) %if any groups selected
    ...
    spikeselectindex = bhdata.grouplist.groupindex{groupselectindex(xx)};
    ncol = 1;
    valuetitle{ncol} = bhdata.grouplist.groupname{groupselectindex(xx)};
    valuecol{ncol} = behav.general.sessID(spikeselectindex); %first column = selected spike names
    for (i = 1:nfield) %search selected sub-fields in each big field
        ...
        for (j = 1:numel(fieldselectindex) )
            ...
            kkk = numel(behav.(fieldtitle{i}).(subfield{i}{nj}));
            for (mm = 1:numel(spikeselectindex))
                if (spikeselectindex(mm) > kkk)
                    valuecol{ncol}{mm} = [];
                elseif (iscell(behav.(fieldtitle{i}).(subfield{i}{nj})))
                    wield = behav.(fieldtitle{i}).(subfield{i}{nj}){spikeselectindex(mm)}; %this is a cell anyway
                    if ( isnumeric (wield) )
                    ...
                    valuecol{ncol}{mm} = behav.(fieldtitle{i}).(subfield{i}{nj})(spikeselectindex(mm));
                end
            end
        end
    end
...
DataManager_behavcontrol_button(hmain, controlpos);
