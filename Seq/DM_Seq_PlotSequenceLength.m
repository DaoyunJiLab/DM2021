function DM_Seq_PlotSequenceLength
...
normflag = 0; Tleng = 1;
ss = questdlg('Normalize sequence length to template length?');
if (strcmp(ss, 'Yes'))
    normflag = 1;
end
evsigval = 0; %2; %1.645;
evtraj = {'selfTracktime'; 'opentime'; 'sleeptime'; 'selftrackspace'}; ntraj = numel(evtraj);
...
for (i = 1:ngroup) %%%%self track running events
    ...
    for (k = 1:numel(cellind{i}))
        if normflag Tleng = tmplength{k}; end
        ...
        if (numel(ii) == 1) 
            [aa, bb] = findeventmeansequencevelocity(datanow, cellind{i}(k), ii, plotparm.significancelevel, ...
            ...
            if (~isempty(aa)) plotval{i,1} = [plotval{i,1} aa/Tleng]; end
            if (~isempty(bb)) plotval{i,4} = [plotval{i,4} bb]; end
        end
    end
end
    ...
    for (k = 1:numel(cellind{i}))
        ...
        for (j = 1:numel(sessnow))
             if (strcmp(sesstypenow{j}, 'open')) && (scorenow(j) > evsigval) 
                ...
                if (~isempty(aa))
                    plotval{i,2} = [plotval{i,2} aa/Tleng];
                end
             elseif (strcmp(sesstypenow{j}, 'sleep')) && (scorenow(j) > evsigval) 
                aa = findsessmeansequencevelocity(datanow, cellind{i}(k), j, plotparm.significancelevel, ...
                    pinfonow, datanow, behav, bhdata, pinfonow.general.finaldir{cellind{i}(k)}, sessname{k}{j});  %%%get the position variation = val(1, nev) but only do it for 'run' type events 
                if (~isempty(aa))
                    plotval{i,3} = [plotval{i,3} aa/Tleng];
                end
             end
        end
    end
...
for (j = 1:ntraj)
    hf = figure('Name', ['SignificantSequencesLength-', evtraj{j}]); wid = 1/ngroup; gap = 0.05;
    mmx = 0; nbin = 30;
